...
class VersionChooser:
    ...
    async def set_remote_versions(self, output: Dict[str, Optional[Union[str, List[Dict[str, Any]]]]], repository: str) -> None:
        try:
            assert isinstance(output["local"], list)
    ...
    async def get_available_local_versions(self) -> web.Response:
        ...
        self.set_local_versions(output)
        return web.json_response(output)

    async def get_available_remote_versions(self, repository: str) -> web.Response:
        output: Dict[str, Optional[Union[str, List[Dict[str, Any]]]]] = {"local": [], "remote": [], "error": None}
        self.set_remote_versions(output, repository)
        return web.json_response(output)
